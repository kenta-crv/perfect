...
# キャッシュを無視してコンテナのビルドを行う ※推奨
rebuild: db-reset build-no-cache
...
run: # コンテナをバックグラウンドで起動
	cp .env.docker .env
	docker-compose up -d
	@echo "\nAccess to http://localhost:8080\nIf you want to connect to develop environment from external host, link http://$(IP):8080"
...
refresh: # refresh
	docker-compose exec app sh -c "docker-compose exec app sh -c"
